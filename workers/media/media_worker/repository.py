            ...
            with self._connect() as conn:
                with conn.transaction():
                    ...
                    for rendition in renditions:
                        conn.execute("insert into video_renditions (id, video_id, label, width, height, target_bitrate, video_codec, segment_count, output_size_bytes, playlist_storage_key, status, created_at) values (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, 'ready', now())", (str(uuid4()), video_id, rendition.label, rendition.width, rendition.height, rendition.target_bitrate, rendition.video_codec, rendition.segment_count, rendition.output_size_bytes, rendition.playlist_storage_key))
                    ...
                    for index, storage_key in enumerate(generated_thumbnail_keys):
                        conn.execute("insert into video_thumbnails (id, video_id, storage_key, source, content_type, width, height, selected, created_at) values (%s, %s, %s, 'generated', 'image/jpeg', 640, 360, %s, now())", (str(uuid4()), video_id, storage_key, index == 0 and not custom_thumbnail_exists))
                    updated = conn.execute(
